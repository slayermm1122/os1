...
This file tracks security work that is intentionally deferred beyond the local-only `v0.02.01` prototype. It is not a claim that OS1 is production-ready.

## Release Boundary

- `v0.02.01` is local-only. HTTP and WebSocket traffic must come from loopback, use a loopback Host, and WebSocket browser traffic must be same-origin.
